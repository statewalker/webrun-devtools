...
2. [Generate API Key](#step-2-generate-api-key): Get an API key generated by the extension to access internal browser APIs. This key stays within your browser for enhanced security.
...
1. *Download the Extension:* Download the prepared extension from CDN (UNPKG: [webrun-devtools-extension.zip](https://unpkg.com/@statewalker/webrun-devtools/dist/webrun-devtools-extension.zip)) and unzip it in a separate folder. Note: see how to build the extension from sources in the section [Build From Sources]( #build-from-sources )
...
Now, the pinned extension icon on the toolbar gives access to the key generation UI. Simply click on the icon to reveal a popup panel managing the key for internal API access.
...
The returned API object provides access to functionalities exposed by the extension. It gives access to browser APIs and allows to perform various automation tasks.
